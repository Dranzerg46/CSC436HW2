...
router.post("/", async function (req, res) {
    const post = new Post({
        ...
        booleanCompleted: req.body.booleanCompleted,
    });
    await post
        .save()
       // .then((document) => {document.id == document._id; document.save()})
        .then((savedPost) => {
            return res.status(201).json({
                ...
                booleanCompleted: savedPost.booleanCompleted,
                id: savedPost.id,
            });
        })
        .catch((error) => {
...
router.patch("/:id", async function(req, res){
    Post.findByIdAndUpdate(req.params.id, {dateCompleted: req.body.dateCompleted, booleanCompleted: req.body.booleanCompleted})
        .where("author")
        .equals(req.payload.id)
        .then((post) => {
            if (post) {
                return res.status(200).json({
                    ...
                    booleanCompleted: post.booleanCompleted,}
                );
